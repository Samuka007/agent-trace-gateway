...
| R12 | D8 养号节律调度：双时钟（节律/需求）+ 单档位 ATCD_NURTURE_LEVEL；突发（burst B 次连发）→燃尽沉寂（quiet+jitter）→天然休整重臂；档位线性决定放弃沉寂余量的比例，地板（min_turn_gap/burst_min_gap）不可让步 | 用户需求（"养号 vs 爆发流量的矛盾，参数化妥协程度"） | 已完成 | docs/atcd-design.md §13（D8 全设计+拒绝的替代）；src/atcd/scheduler.rs 状态机 + 7 单测（突发/沉寂/档位 0-0.5-1/重臂/并发界）；env 接线 ATCD_NURTURE_LEVEL 等 5 参数；全量 55 passed 0 failed |
