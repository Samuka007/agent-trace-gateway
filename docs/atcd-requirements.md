...
| R13 | 多设备建模（D9 提案）：同一账号 1:N installations（installation=一次登录/一台设备，codex 语义即如此，v4 无时间戳不可鉴别）；新会话绑定择一 installation；三个投影点读绑定值；PacingGate 按 installation 记账；旋钮 K=安装数（默认 1=行为不变）。动机：多设备是真实用法（保真修正），且每设备独立节律 → 瞬时并发 ×K；聚合护栏留 1.3~1.6x 而非满 2x，用配额头燃烧速度实测调 | 用户提案（"两个 installation id → 两倍的设计并发？"） | 待决策 | 事实依据：codex installation_id.rs:19-64（v4、每安装目录一份、无登记册）；persona.rs:80 同为 v4；现语义为每账号固定一个、重登录会覆盖（需改为追加） |
| R12 | D8 养号节律调度：两套时间尺度（养号节奏/排队等待）+ 参数 ATCD_NURTURE_LEVEL；突发（B 次连发）→燃尽沉寂（quiet+jitter）→天然休整恢复额度；参数线性决定放弃沉寂余量的比例，最小间隔（min_turn_gap/burst_min_gap）不可让步 | 用户需求（"养号 vs 爆发流量的矛盾，参数化妥协程度"） | 已完成 | docs/atcd-design.md §13（D8 全设计+拒绝的替代）；src/atcd/scheduler.rs 状态机 + 7 单测（突发/沉寂/档位 0-0.5-1/额度恢复/并发界）；env 接线 ATCD_NURTURE_LEVEL 等 5 参数；全量 55 passed 0 failed |
