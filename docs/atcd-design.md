...
- **节律时钟（养号）**：账号级状态机 `burst → quiet → rest → re-arm`。
  人类的工作模式是「一段专注里连点几下（2~3 个请求搭在一起），然后离开
  一段时间」——突发 + 长尾沉寂，而不是恒定速率。恒定速率本身就是机器特征。
- **需求时钟（排队）**：等待中的请求的等待时长。现有架构没有显式队列——
  背压 = 请求在 `wait_turn` 里等待；档位只改变等待时长，不改变排队点。
...
1. **天然休整**（`now ≥ last_turn+gap+jitter` 且 `now ≥ quiet_until`）：
   放行，且**重臂** `burst_left = B`（用户休息后回来，又能快速连发）。
...
1. **地板不可让步**：`min_turn_gap`（1200ms）与 `burst_min_gap`（250ms）
   永远执行——亚秒级规律性是最强的机器信号。档位只调节「歇息余量」
   （quiet），不调节人类地板。这是对"多快放弃沉寂"的精确回答：
   放弃的只是沉寂，不是底线。
...
| `ATCD_BURST_MIN_GAP_MS` | 250 | 突发内间隔地板 |
| `ATCD_QUIET_MS` | 8000 | 燃尽后沉寂基数 |
| `ATCD_QUIET_JITTER_MS` | 3000 | 沉寂抖动 |
| （既存）`ATCD_MIN_TURN_GAP_MS` | 1200 | 休整间隔地板 |
