...
## 14. D9（提案，待决策）：installation 降为设备级——一个账号、多台设备

> 用户提案："同一个账号，两个 installation id -> 两倍的设计并发？"
> 状态：**提案**，未实现。本节记录事实、设计草案与风险边界，待决策后转正。

### 14.1 事实（有源码依据）

1. `installation_id` 是 **UUID v4**（`core/src/installation_id.rs:54`），
   存放于每个 codex 安装目录的 `installation_id` 文件，首次运行生成、之后复用
   （:19-64）。语义 = **一台设备上的一个安装**；同账号多设备 = 多个 installation。
2. 无服务端登记册：该值纯客户端自报；v4 无时间戳，mint 产物与真实设备不可区分。
3. 现有建模是"每账号一个"（`persona.rs:80`，登录时生成，存账号行）；
   重复 `atcd login` 会**覆盖**而非追加——多设备能力需要先修这个语义。
4. installation 出现在三个投影点（headers / turn 元数据 / body client_metadata），
   均已在外科替换范围内（B4）。

### 14.2 设计草案

- **语义**：installation = 一次登录（一台设备）。账号 1:N。
- **绑定**：新会话绑账号时同时择一 installation（轮转/最久未用，与账号选择同构）。
- **投影**：三个投影点从会话绑定的 installation 取值（B4 修订：installation
  从账号级降为设备级，仍拒绝"重铸人设"——同账号身份不换，只多设备）。
- **节奏**：`PacingGate` 按 installation 记账（各自连发/休息/被放弃的休息）；
  账号可加一条更松的聚合护栏（初值 1.3~1.6x，不做满 2x）。
- **旋钮**：每账号安装数 K（默认 1，行为与现状一致）。

### 14.3 风险与观测

- 瞬时并发 ×K 有正当性（两个独立节律交错更像人）；**聚合吞吐**是唯一可能
  异常的维度，可用现成信号闭环：配额头（5h/7d）燃烧速度、可服务人数。
- 观测先行：先开 K=2 于少量账号，观察配额燃烧与账号状态，再定护栏值。
