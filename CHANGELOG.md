...
**bench 验证结果（2026-09-16，见 #13 验收记录）**：故障复现通过——bench fake
384/s 下，基线三窗丢 2437/2203/2013（各 9600 turn，队列打满 1024），本版三窗
**0/0/0**；接收端独立点数与 `exported` 增量精确相等；19/19 窗守恒残差 0。
并发度扫描（`ATG_EXPORT_MAX_INFLIGHT` 1/2/4/8）：拐点在 2，默认 8 保留
（内存有界 8×~15.7 MB，远低于实测噪声底）。
**注意**：229 → ≥820 turns/s 的容量口径**未验证**——测试台 driver 在 384/s 以上
无法继续加压（仪器限制，见 scitrace-loadlab#21）；可声明的是「在测试台可达的
最高压力下，导出不再是瓶颈」，820 不作为已达成容量写入任何发版口径。
...
- `PrefixStitcher::assign` 原先**每请求**在进程级锁内做一次 `retain` 全表 TTL
  清扫（O(表规模)）。release 实测单次调用：表 0/5k/20k/100k 链 =
  9.2 / 81.8 / 240.9 / 1048.0 µs；8 线程、表 5 万链、c=64 下吞吐
  9050 → 784.6 rps、TTFB p50 6.55 → 79.86 ms（同二进制同负载，仅表规模不同）。
- 现改为**摊还**：清扫每 30s 至多一次（`SWEEP_INTERVAL_MS`，锁内一次 relaxed
  原子读判定），过期在查表时**逐条判定**（语义不变：过期链既不续接也不复用旧
  session id）；指纹计算本就在锁外，容量/LRU 淘汰逻辑未动。同一对照：
  9543 rps / TTFB p50 6.42 ms，表规模不再影响请求路径。
- 定位说明：这是**表长大后不撞容量墙**的修复；8 线程下 ~1.8× 的每请求 CPU 放大
  不由此解释（该速率下清扫 duty 仅 ~3%）。
- 新增 5 条单测（摊还不变量 / 活链延伸 / 过期不续接 / 到期清扫回收 / 容量上界）、
  按需仪器 `tests/stitch_sweep_cost.rs`，以及测量配方 `tools/perf/`（并发假上游、
  驱动、单臂脚本、消融助手 + 纪律 README）。
