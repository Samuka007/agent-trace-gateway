...
async fn prefix_stitch() {
    ...
    let fixture_dir = format!("{}/xtask/harness/fixtures/openai.chat_completions", manifest_dir());
    ...
    let chat: Vec<_> = recs
        ...
        .collect();
    assert_eq!(chat.len(), 5, "five openai.chat_completions records expected: {recs:?}");
